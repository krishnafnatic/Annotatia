...
if($con->connect_error){
    echo 'Connection Faild: '.$con->connect_error;
        ...
        $sql="select * from upload where email like '%$search_value%'";
        $res=$con->query($sql);
		
        while($row=$res->fetch_assoc()){?>
           <font style='color:black'><h3>User :  </h3><?php echo $row['email'];?></a></font>
		   <br>
	<a style="color:black" href="download.php?filename=<?php echo $row['name'] ?>">File : <?php echo $row['name'] ?></a>
			<br>
		<?php	echo "<br>";
